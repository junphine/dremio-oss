...
public final class JdbcSortRule extends JdbcUnaryConverterRule
{
    ...
    public boolean matches(final RelOptRuleCall call) {
        ...
        JdbcSortRule.logger.debug("Checking if Sort node {} is supported using supportsSort().", (Object)sort);
        final boolean isSortSupported = JdbcConverterRule.getDialect(pluginId).supportsSort(sort);
        if (!isSortSupported) {
            JdbcSortRule.logger.debug("Sort '{}' is unsupported. Aborting pushdown.", (Object)sort);
            return false;
        }
        JdbcSortRule.logger.debug("Sort '{}' is supported.", (Object)sort);
        return true;
    }
