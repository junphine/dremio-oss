...
public class OperatorDescriptor
{
    ...
    public boolean equals(final Object o) {
        ...
        return rhs.arguments.isEmpty() || this.arguments.isEmpty() || rhs.arguments.get(0).equals((Object)this.arguments.get(0));
    }
    ...
    static OperatorDescriptor createFromRexCall(final RexCall call, final CallTransformer transformer, final boolean isDistinct, final boolean isVarags) {
        final List<RelDataType> argTypes = (List<RelDataType>)transformer.transformRexOperands((List)call.operands).stream().map(RexNode::getType).collect(Collectors.toList());
    ...
    static OperatorDescriptor createFromRelTypes(String name, final boolean isDistinct, final RelDataType returnType, final List<RelDataType> argTypes, final boolean isVarArgs) {
        ...
        final List<CompleteType> argsAsCompleteTypes = argTypes.stream().map((Function<? super Object, ?>)SourceTypeDescriptor::getType).map(t -> t.isDecimal() ? OperatorDescriptor.SIMPLE_DECIMAL : t).collect((Collector<? super Object, ?, List<CompleteType>>)Collectors.toList());
        if (isDistinct) {
            name += "_distinct";
        }
    ...
    public String toString() {
        final StringBuilder sb = new StringBuilder().append("Operator name: '").append(this.name).append("'\n").append("Is varargs: ").append(this.isVarArgs).append("\n").append("Argument types: ").append(this.arguments.stream().map((Function<? super Object, ?>)CompleteType::toString).collect((Collector<? super Object, ?, String>)Collectors.joining(", "))).append("\n");
    ...
    static {
        ...
        operatorNameToArpNameMap = (Map)builder.put((Object)"DATETIME_PLUS", (Object)"+").put((Object)"DATETIME_MINUS", (Object)"-").put((Object)"$SUM0", (Object)"SUM").build();
    }
}
