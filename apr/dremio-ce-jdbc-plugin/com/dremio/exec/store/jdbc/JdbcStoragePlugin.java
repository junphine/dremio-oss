...
public class JdbcStoragePlugin implements StoragePlugin, SourceMetadata, SupportsListingDatasets, SupportsExternalQuery
{
    ...
    public SourceState getState() {
        if (null == this.source) {
            JdbcStoragePlugin.LOGGER.error("JDBC source {} has not been started.", (Object)this.name);
    ...
    public PhysicalOperator getExternalQueryPhysicalOperator(final PhysicalPlanCreator creator, final ExternalQueryScanPrel prel, final BatchSchema schema, final String sql) {
        ...
        final ImmutableSet.Builder<String> skippedColumnsBuilder = (ImmutableSet.Builder<String>)new ImmutableSet.Builder();
        ...
        return (PhysicalOperator)new JdbcGroupScan(creator.props((Prel)prel, "$dremio$", schema, JdbcPrel.RESERVE, JdbcPrel.LIMIT), sql, (List<SchemaPath>)filteredSchema.getFields().stream().map(f -> SchemaPath.getSimplePath(f.getName())).collect(ImmutableList.toImmutableList()), this.getPluginId(), filteredSchema, (Set<String>)skippedColumns);
    }
    ...
    private void filterBatchSchema(final BatchSchema originalSchema, final SchemaBuilder filteredSchemaBuilder, final ImmutableSet.Builder<String> skippedColumnsBuilder) {
        for (final Field field : originalSchema) {
            if (field.getType().getTypeID() == ArrowType.Null.TYPE_TYPE) {
                skippedColumnsBuilder.add((Object)field.getName());
            }
            else {
    ...
    public static final class Config
    {
        ...
        private Config(final Builder builder) {
            this.dialect = (JdbcDremioSqlDialect)Preconditions.checkNotNull((Object)builder.dialect);
            this.datasourceFactory = (CloseableDataSource.Factory)Preconditions.checkNotNull((Object)builder.datasourceFactory);
        ...
        public static final class Builder
        {
            ...
            private static Set<String> newUnmodifiableSet(final String... strings) {
                return Collections.unmodifiableSet((Set<? extends String>)Arrays.stream(strings).collect((Collector<? super String, ?, Set<? extends T>>)Collectors.toCollection(() -> new TreeSet((Comparator<? super Object>)String.CASE_INSENSITIVE_ORDER))));
            }
            ...
            public Builder withDialect(final JdbcDremioSqlDialect dialect) {
                this.dialect = (JdbcDremioSqlDialect)Preconditions.checkNotNull((Object)dialect);
                return this;
            }
