...
public final class JdbcExpressionSupportCheck implements RexVisitor<Boolean>
{
    ...
    public Boolean visitCall(final RexCall paramRexCall) {
        ...
        List<RelDataType> paramTypes = operands.stream().map(RexNode::getType).collect(Collectors.toList());
        ...
        if (transformer == TimeUnitFunctionTransformer.INSTANCE) {
            JdbcExpressionSupportCheck.logger.debug("Operator {} has been identified as a time unit function. Checking support using supportsTimeUnitFunction().", paramRexCall.getOperator().getName());
            ...
            if (transformer != NoOpTransformer.INSTANCE) {
                ...
                paramTypes = (List<RelDataType>)transformer.transformRexOperands(paramRexCall.operands).stream().map(RexNode::getType).collect(Collectors.toList());
            }
            else {
                operator = paramRexCall.getOperator();
            }
            JdbcExpressionSupportCheck.logger.debug("Verifying support for operator {} using supportsFunction().", paramRexCall.getOperator().getName());
        ...
        if (supportsFunction) {
            ...
            for (final RexNode operand : operands) {
                ...
                if (!(boolean)operand.accept((RexVisitor)this)) {
                    JdbcExpressionSupportCheck.logger.debug("Operand {} for operator {} was not supported. Aborting pushdown.", i, paramRexCall.getOperator().getName());
                    return false;
                }
            }
            JdbcExpressionSupportCheck.logger.debug("Operator {} was supported.", paramRexCall.getOperator().getName());
            return true;
        }
        ...
        JdbcExpressionSupportCheck.logger.debug("Operator {} was not supported. Aborting pushdown of a RelNode using this operator.", paramRexCall.getOperator().getName());
        return false;
    }
    ...
    public Boolean visitLiteral(final RexLiteral literal) {
        ...
        JdbcExpressionSupportCheck.logger.debug("Literal of type {} encountered. Calling supportsLiteral().", literal.getType());
    ...
    private Boolean visitDatetimePlusAsTimestampAdd(final RexCall call) {
        ...
        if (!this.dialect.supportsTimeUnitFunction((SqlOperator)SqlStdOperatorTable.TIMESTAMP_ADD, timeUnit, call.getType(), (List)ImmutableList.of(factory.createSqlType(SqlTypeName.ANY), factory.createSqlType(SqlTypeName.INTEGER), call.getOperands().get(0).getType()))) {
    ...
    private boolean canReplaceIntervalMultiplicationWithIntegerMultiplication(final RexNode coefficient) {
        ...
        final RelDataType multiplyDataType = this.builder.deriveReturnType((SqlOperator)SqlStdOperatorTable.MULTIPLY, (List)ImmutableList.of(coefficient, this.builder.makeExactLiteral(BigDecimal.ONE)));
        if (!this.dialect.supportsFunction((SqlOperator)SqlStdOperatorTable.MULTIPLY, multiplyDataType, (List)ImmutableList.of(coefficient.getType(), this.builder.getTypeFactory().createSqlType(SqlTypeName.INTEGER)))) {
            JdbcExpressionSupportCheck.logger.debug("Failed to pushdown conversion of DATETIME_PLUS to TIMESTAMPADD.");
            return false;
        }
    ...
    private boolean supportsDateTimeFormatString(final SqlOperator operator, final List<RexNode> operands, final int index) {
        if (operands.size() < index || !(operands.get(index) instanceof RexLiteral)) {
            JdbcExpressionSupportCheck.logger.debug("Operator {} was not supported due to a non-string literal value. Aborting pushdown.", operator.getName());
            return false;
        }
        ...
        JdbcExpressionSupportCheck.logger.debug("Operator {} has been identified as having a datetime format string. Checking support using supportsDateTimeFormatString().", operator.getName());
