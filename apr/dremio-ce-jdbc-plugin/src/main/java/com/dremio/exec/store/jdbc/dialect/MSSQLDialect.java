...
public class MSSQLDialect extends ArpDialect
{
    ...
    private static final class MSSQLSchemaFetcher extends ArpSchemaFetcher
    {
        ...
        public long getRowCount(final JdbcDatasetHandle handle) {
            ...
            MSSQLSchemaFetcher.logger.debug("Row count estimate {} detected on table {}. Retrying with count_big query.", 1000000000L, quotedPath);
            final java.util.Optional<Long> fallbackEstimate = this.executeQueryAndGetFirstLong("SELECT COUNT_BIG(*) FROM " + quotedPath);
