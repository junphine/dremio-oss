...
public final class ArpYaml
{
    ...
    public SqlNode getCastSpec(final RelDataType type) {
        ...
        if (mapping == null) {
            ArpYaml.logger.debug("No cast spec found for type: '{}'", type);
            return null;
        }
    ...
    public SqlNode getSqlNodeForOperator(final SqlCall sqlCall, final RexCall rexCall, final CallTransformer transformer) {
        if (ArpYaml.logger.isDebugEnabled()) {
            ArpYaml.logger.debug("Searching for scalar operator {} or windowed aggregate for SQL generation.", rexCall);
        }
        Signature sig;
        if (sqlCall.isA((Set)SqlKind.AGGREGATE)) {
            ...
            ArpYaml.logger.debug("Searching in aggregation/functions YAML section for operator {}", op);
            ...
            ArpYaml.logger.debug("Searching in operators YAML section for operator {}", op);
            sig = this.expressions.getOperators().get(op);
            if (sig == null) {
                op = OperatorDescriptor.createFromRexCall(rexCall, transformer, false, true);
                ArpYaml.logger.debug("Searching in variable_length_operators YAML section for operator {}", op);
        ...
        ArpYaml.logger.debug("Applying rewrite during unparsing: {}", sig);
    ...
    public SqlNode getSqlNodeForOperator(final SqlCall undecoratedNode, final AggregateCall aggCall, final List<RelDataType> types) {
        if (ArpYaml.logger.isDebugEnabled()) {
            ArpYaml.logger.debug("Searching for aggregate function {} with params {} for SQL generation.", aggCall.getName(), types);
        }
    ...
    public boolean supportsScalarOperator(final SqlOperator operator, final List<RelDataType> argTypes, final RelDataType returnType) {
        ArpYaml.logger.debug("Identifying if operator {} is supported.", operator.getName());
        ...
        ArpYaml.logger.debug("Searching for operator {} in expressions/operators section in YAML.", op);
        ...
        if (!supportsOperator) {
            ...
            ArpYaml.logger.debug("Searching for operator {} in variable_length_operators section in YAML.", op);
            supportsOperator = this.expressions.getVariableOperators().containsKey(op);
        }
        if (!supportsOperator) {
            ArpYaml.logger.debug("Operator {} not supported. Aborting pushdown.", operator.getName());
        }
        else {
            ArpYaml.logger.debug("Operator {} supported.", operator.getName());
        }
    ...
    public boolean supportsAggregate(final SqlOperator operator, final boolean isDistinct, final List<RelDataType> argTypes, final RelDataType returnType) {
        ...
        ArpYaml.logger.debug("Searching aggregation/functions in YAML for aggregate {}", op);
        final boolean supportsAggregate = this.relationalAlgebra.getAggregation().getFunctionMap().containsKey(op);
        if (!supportsAggregate) {
            ArpYaml.logger.debug("Aggregate {} not supported. Aborting pushdown.", op);
        }
        else {
            ArpYaml.logger.debug("Aggregate {} supported.", op);
        }
    ...
    public boolean supportsTimeUnitFunction(final SqlOperator operator, final TimeUnitRange unit, final List<RelDataType> argTypes, final RelDataType returnType) {
        ...
        ArpYaml.logger.debug("Checking expressions/operators the time unit-based function {} is supported.", op);
        final boolean isSupported = this.expressions.getOperators().containsKey(op);
        if (!isSupported) {
            ArpYaml.logger.debug("Time unit {} not supported. Aborting pushdown.", op);
        }
        else {
            ArpYaml.logger.debug("Time unit function {} supported.", op);
        }
        return isSupported;
    }
    ...
    public boolean supportsCountOperation(final AggregateCall call) {
        ...
        ArpYaml.logger.debug("Searching count_functions section in YAML for {}.", opType);
    ...
    private SqlNode getNodeForCountOperation(final SqlCall call) {
        ...
        ArpYaml.logger.debug("Count variable rewrite: {}", sig);
    ...
    public JoinOp getJoinSupport(final JoinType joinType) {
        ...
        ArpYaml.logger.debug("Searching join section in YAML for join type '{}', found {}", joinType, joinOp);
        return joinOp;
    }
