...
public class JdbcSchemaFetcher
{
    ...
    protected long getRowCount(final JdbcDatasetHandle handle) {
        ...
        JdbcSchemaFetcher.logger.debug("Getting row count for table {}. ", quotedPath);
        final Optional<Long> count = this.executeQueryAndGetFirstLong("select count(*) from " + this.getQuotedPath(handle.getIdentifiers()));
        if (count.isPresent()) {
            return count.get();
        }
        JdbcSchemaFetcher.logger.debug("There was a problem getting the row count for table {}, using default of {}.", quotedPath, 1000000000L);
        return 1000000000L;
    }
    
    public DatasetHandleListing getTableHandles() {
        if (this.config.shouldSkipSchemaDiscovery()) {
            JdbcSchemaFetcher.logger.debug("Skip schema discovery enabled, skipping getting tables '{}'", this.storagePluginName);
    ...
    public Optional<DatasetHandle> getTableHandle(final List<String> tableSchemaPath) {
        try {
            ...
            JdbcSchemaFetcher.logger.warn("Failed to fetch schema for {}.", tableSchemaPath);
                ...
                try {
                    ...
                    try {
                        ...
                        if (colCount != 1 || !resultSet.next()) {
                            JdbcSchemaFetcher.logger.debug("Invalid results returned for `{}`, colCount = {}.", sql, colCount);
                            return Optional.empty();
                        }
                        ...
                        JdbcSchemaFetcher.logger.debug("Query `{}` returned {} rows.", sql, numRows);
    ...
    protected static List<String> getSchemas(final DatabaseMetaData metaData, final String catalogName, final JdbcStoragePlugin.Config config, final List<String> failed) {
        ...
        JdbcSchemaFetcher.logger.debug("Getting schemas for catalog=[{}].", catalogName);
        try {
    ...
    private List<String> getCatalogsOrSchemas() {
        if (this.config.showOnlyConnDatabase() && this.config.getDatabase() != null) {
            return (List<String>)ImmutableList.of(this.config.getDatabase());
        }
        try {
            final Connection connection = this.dataSource.getConnection();
            Throwable x0 = null;
            try {
                ...
                final ImmutableList.Builder<String> catalogs = ImmutableList.builder();
                JdbcSchemaFetcher.logger.debug("Getting catalogs from JDBC source {}", this.storagePluginName);
    ...
    private Optional<DatasetHandle> getDatasetHandleViaGetTables(final List<String> tableSchemaPath, final Connection connection) throws SQLException {
        ...
        try {
            while (tablesResult.next()) {
                ...
                //modify@byron
                if (supportsCatalogs(this.config.getDialect(), metaData) && !Strings.isNullOrEmpty(currCatalog)) {
                ...
                return Optional.of(new JdbcDatasetHandle(new EntityPath((List)pathBuilder.build())));
            }
        }
        catch (Throwable t) {
            x0 = t;
        ...
        try {
            ...
            if (supportsCatalogs(this.config.getDialect(), metaData)) {
                ...
                if (!Strings.isNullOrEmpty(catalog)) {
                    pathBuilder.add(catalog);
                }
            }
            if (supportsSchemas(this.config.getDialect(), metaData)) {
                final String schema = preparedMetadata.getSchemaName(1);
                if (!Strings.isNullOrEmpty(schema)) {
                    pathBuilder.add(schema);
                }
            }
            final String table = preparedMetadata.getTableName(1);
            if (Strings.isNullOrEmpty(table)) {
                JdbcSchemaFetcher.logger.info("Unable to get table handle for {} via prepare, falling back to getTables.", this.getQuotedPath(tableSchemaPath));
                return this.getDatasetHandleViaGetTables(tableSchemaPath, connection);
            }
            pathBuilder.add(table);
            return Optional.of(new JdbcDatasetHandle(new EntityPath(pathBuilder.build())));
        }
        catch (Throwable t) {
            x0 = t;
            throw t;
    ...
    protected static class FilterDescriptor
    {
        ...
        public FilterDescriptor(final List<String> tableSchemaPath, final boolean hasCatalogsWithoutSchemas) {
            ...
            if (tableIdentifier.size() == 1) {
                ...
                this.catalogName = (hasCatalogsWithoutSchemas ? tableIdentifier.get(0) : null);
            ...
            try {
                ...
                if (config.getDatabase() != null && config.showOnlyConnDatabase()) {
                    if (this.supportsCatalogs) {
                        this.catalogs = (Iterator<String>)ImmutableList.of(config.getDatabase()).iterator();
                    }
