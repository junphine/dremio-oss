...
public final class JdbcUnionRule extends JdbcBinaryConverterRule
{
    ...
    public boolean matchImpl(final RelOptRuleCall call) {
        ...
        JdbcUnionRule.logger.debug("Checking if Union is supported on dialect using supportsUnion{}", (union.all ? "All" : ""));
    ...
    public RelNode convert(final RelNode rel, final JdbcCrel left, final JdbcCrel right, final StoragePluginId pluginId) {
        ...
        return (RelNode)new JdbcUnion(rel.getCluster(), traitSet.replace((RelTrait)Rel.LOGICAL), (List<RelNode>)ImmutableList.of(left.getInput(), right.getInput()), union.all, pluginId);
    }
