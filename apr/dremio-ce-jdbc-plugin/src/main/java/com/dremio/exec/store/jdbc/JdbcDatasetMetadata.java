...
public class JdbcDatasetMetadata implements DatasetMetadata, PartitionChunkListing
{
    ...
    public Schema getRecordSchema() {
        this.buildIfNecessary();
        return new Schema(this.jdbcToFieldMappings.stream().map(JdbcToFieldMapping::getField).collect(Collectors.toList()));
    }
    ...
    private void buildIfNecessary() {
        if (this.jdbcToFieldMappings == null) {
            ...
            JdbcDatasetMetadata.logger.info("Took {} ms to get column metadata for {}", watch.elapsed(TimeUnit.MILLISECONDS), this.handle.getDatasetPath());
        }
    }
    ...
    private void getColumnMetadata(final MapFunction mapFields) {
        try {
            ...
            try {
                ...
                final ImmutableList.Builder<String> skippedColumnBuilder = ImmutableList.builder();
                final ListMultimap<String, JdbcReaderProto.ColumnProperty> properties = ArrayListMultimap.create();
                this.jdbcToFieldMappings = mapFields.map(connection, filters, skippedColumnBuilder, properties);
                final ImmutableList.Builder<JdbcReaderProto.ColumnProperties> columnPropertiesListBuilder = ImmutableList.builder();
