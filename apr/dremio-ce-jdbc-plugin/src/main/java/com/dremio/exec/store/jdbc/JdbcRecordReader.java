...
public final class JdbcRecordReader extends AbstractRecordReader
{
    ...
    public void setup(final OutputMutator output) {
        ...
        try {
            ...
            final ImmutableList.Builder<ValueVector> vectorBuilder = ImmutableList.builder();
            final ImmutableList.Builder<Copier<?>> copierBuilder = ImmutableList.builder();
            ...
            final ImmutableList.Builder<ValueVector> valueVectorBuilder = vectorBuilder;
            final ImmutableList.Builder<Copier<?>> copierBuilder2 = copierBuilder;
    ...
    public void cancel() {
        ...
        try {
            ...
            JdbcRecordReader.logger.info("Call to cancel query on RDBMs source failed with error: {}", e.getMessage());
            JdbcRecordReader.logger.debug("Call to cancel query on RDBMs source failed with error: {}", e.getMessage(), e);
        }
    }
    ...
    public void checkSchemaConsistency(final List<JdbcToFieldMapping> mappings) {
        if (mappings.size() > this.columns.size()) {
            ...
            if (!unexpectedColumns.isEmpty()) {
                builder.addContext("Unexpected columns: %s", Strings.join(unexpectedColumns, ","));
            }
            throw builder.addContext("Expected skipped columns: ", Strings.join(this.skippedColumns, ",")).setAdditionalExceptionContext((AdditionalExceptionContext)new InvalidMetadataErrorContext((List)ImmutableList.copyOf((Collection)this.tableList))).build(JdbcRecordReader.logger);
        }
        final List<String> mappingsFieldNames = mappings.stream().map(m -> m.getField().getName().toLowerCase(Locale.ROOT)).collect(Collectors.toList());
    ...
    private List<String> identifyUnexpectedColumns(final List<JdbcToFieldMapping> jdbcMappings) {
        final List<String> expectedColumns = this.columns.stream().map(BasePath::getAsUnescapedPath).map(String::toLowerCase).collect(Collectors.toList());
        final ImmutableList.Builder<String> unexpectedColumnListBuilder = ImmutableList.builder();
        for (final JdbcToFieldMapping mapping : jdbcMappings) {
            ...
            if (!expectedColumns.contains(columnName) && !this.skippedColumns.contains(columnName)) {
                JdbcRecordReader.logger.debug("Unrecognized column name found: {}.", columnName);
