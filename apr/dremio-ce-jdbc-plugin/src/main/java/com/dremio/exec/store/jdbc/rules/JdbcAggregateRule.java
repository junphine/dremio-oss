...
public class JdbcAggregateRule extends JdbcUnaryConverterRule
{
    ...
    public boolean matches(final RelOptRuleCall call) {
        ...
        for (final AggregateCall aggCall : aggregate.getAggCallList()) {
            JdbcAggregateRule.logger.debug("Aggregate expression: {}", aggCall);
            ...
            if (aggCall.getAggregation().getKind() == SqlKind.COUNT) {
                ...
                if (!dialect.supportsAggregateFunction(aggCall.getAggregation().getKind())) {
                    JdbcAggregateRule.logger.debug("Aggregate function {} not supported by dialect. Aborting pushdown.", aggCall.getAggregation().getName());
                    return false;
                }
                final List<RelDataType> types = (List<RelDataType>)SqlTypeUtil.projectTypes(aggregate.getInput().getRowType(), aggCall.getArgList());
                JdbcAggregateRule.logger.debug("Checking if aggregate function {} used with types {} is supported by dialect using supportsFunction.", aggCall.getAggregation().getName(), types);
                if (!dialect.supportsFunction(aggCall, (List)types)) {
                    JdbcAggregateRule.logger.debug("Aggregate {} with type {} not supported by dialect. Aborting pushdown", aggCall.getAggregation().getName(), types);
                    return false;
                }
                continue;
            }
