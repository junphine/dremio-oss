...
public class MongoChunks implements Iterable<MongoChunk>
{
    ...
    public MongoChunks(final MongoCollection collection, final MongoConnection connection, final MongoTopology topology, final int subpartitionSize, final String pluginName) {
        ...
        try {
            ...
            MongoChunks.logger.debug("Mongo chunks retrieved in {} ms.", (Object)watch.elapsed(TimeUnit.MILLISECONDS));
        }
    }
    ...
    private static boolean isHashed(final com.mongodb.client.MongoCollection<Document> collection) {
        try {
            for (final Document oneIndex : collection.listIndexes()) {
                for (final Map.Entry<String, Object> oneKey : ((Document)oneIndex.get((Object)"key")).entrySet()) {
                    if (oneKey.getValue() instanceof String && "hashed".equalsIgnoreCase(oneKey.getValue())) {
                        return true;
                    }
                }
            }
    ...
    private static boolean isShardedCollection(final MongoDatabase db, final String collectionName) {
        final Document stats = MongoDocumentHelper.runMongoCommand(db, (Bson)new Document("collStats", (Object)collectionName));
        return stats != null && stats.getBoolean((Object)"sharded", false);
    }
    ...
    private boolean isCompoundKey(final MongoConnection client) {
        ...
        final Document minMap = (Document)firstChunk.get((Object)"min");
        final Document maxMap = (Document)firstChunk.get((Object)"max");
    ...
    private FindIterable<Document> getChunkCollection(final MongoConnection client) {
        ...
        final Document filter = new Document("ns", (Object)this.collection.toName());
        final Document projection = new Document().append("shard", (Object)MongoChunks.SELECT).append("min", (Object)MongoChunks.SELECT).append("max", (Object)MongoChunks.SELECT);
    ...
    private List<MongoChunk> handleShardedCollection(final MongoConnection connection) {
        ...
        final Map<String, MongoChunk> addresses = (Map<String, MongoChunk>)Maps.newHashMap();
        ...
        while (iterator.hasNext()) {
            ...
            final String shardName = (String)chunkObj.get((Object)"shard");
            final MongoChunk chunk = new MongoChunk(addresses.get(shardName).getChunkLocList());
            final Document minMap = (Document)chunkObj.get((Object)"min");
            chunk.setMinFilters(minMap);
            final Document maxMap = (Document)chunkObj.get((Object)"max");
    ...
    static {
        SELECT = 1;
        logger = LoggerFactory.getLogger((Class)MongoChunks.class);
    }
}
