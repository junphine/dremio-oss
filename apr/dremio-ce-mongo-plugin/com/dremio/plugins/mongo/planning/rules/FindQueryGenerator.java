...
public class FindQueryGenerator extends RexVisitorImpl<Document>
{
    ...
    private Document handleFindQueryComparison(final RexCall call, final String funcName) {
        ...
        final Document filter = new Document(compoundFieldName, (Object)new Document(mongoFunction, otherArg));
    ...
    private static Document addNotNullCheck(final Document filter, final String mongoFormattedColReference) {
        final Document notNullFilter = new Document(mongoFormattedColReference, (Object)new Document(MongoFunctions.NOT_EQUAL.getMongoOperator(), (Object)null));
        return new Document(MongoFunctions.AND.getMongoOperator(), (Object)Arrays.asList(notNullFilter, filter));
    }
    ...
    private Document handleIsNullFunction(final RexCall call, final boolean isNull) {
        ...
        if (firstOp instanceof RexInputRef) {
            ...
            return new Document(compoundFieldName, (Object)new Document(mongoFunction, (Object)null));
        }
        return this.getNodeAsAggregateExprDoc((RexNode)call);
    }
    
    static {
        logger = LoggerFactory.getLogger((Class)FindQueryGenerator.class);
    }
}
