...
public class MongoIntermediateScanPrel extends ScanPrelBase implements MongoRel
{
    public MongoIntermediateScanPrel(final RelOptCluster cluster, final RelTraitSet traitSet, final RelOptTable table, final TableMetadata tableMetadata, final List<SchemaPath> projectedColumns, final double observedRowcountAdjustment) {
        super(cluster, traits(cluster, table.getRowCount(), tableMetadata.getSplitCount(), traitSet, tableMetadata), table, tableMetadata.getStoragePluginId(), tableMetadata, (List)projectedColumns, observedRowcountAdjustment);
    }
    ...
    public BatchSchema getSchema(final FunctionLookupContext context) {
        ...
        return this.tableMetadata.getSchema().maskAndReorder((List)ImmutableList.copyOf((Collection)paths));
    }
    ...
    public MongoScanSpec implement(final MongoImplementor impl) {
        ...
        for (final SchemaPath col : newColumns) {
            columnsDoc.put(col.getAsUnescapedPath(), (Object)1);
        }
        return spec.plusPipeline(Collections.singletonList(new Document(MongoPipelineOperators.PROJECT.getOperator(), (Object)columnsDoc)), false);
    }
}
