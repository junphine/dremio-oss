...
public class MongoSubpartitioner
{
    ...
    public List<Range> getPartitions(final MongoCollection collection, final int recordsPerCollection) {
        ...
        final com.mongodb.client.MongoCollection<Document> documents = (com.mongodb.client.MongoCollection<Document>)database.getCollection(collection.getCollection());
        final long totalDocs = documents.count();
        if (totalDocs < recordsPerCollection) {
            final List<Range> oneRange = (List<Range>)Lists.newArrayList();
            oneRange.add(new Range(new MinKey(), new MaxKey()));
            return oneRange;
        }
        final List<Range> ranges = (List<Range>)Lists.newArrayList();
    ...
    private Object getValueCondition(final com.mongodb.client.MongoCollection<Document> collection, final int skip) {
        final Document o = (Document)collection.find().projection((Bson)new Document("_id", (Object)1)).sort((Bson)new Document("_id", (Object)1)).skip(skip).first();
        if (o == null) {
            return new MaxKey();
        }
