...
public class MongoPipeline
{
    ...
    public List<Document> getPipelines() {
        final List<Document> pipelineToReturn = (List<Document>)Lists.newArrayListWithCapacity(2);
        if (this.match != null) {
            pipelineToReturn.add(new Document(MongoPipelineOperators.MATCH.getOperator(), (Object)this.match));
        }
        if (this.project != null) {
            pipelineToReturn.add(new Document(MongoPipelineOperators.PROJECT.getOperator(), (Object)this.project));
        }
    ...
    public boolean hasProject() {
        return this.getPipelines().stream().anyMatch(p -> p.get((Object)MongoPipelineOperators.PROJECT.getOperator()) != null);
    }
    ...
    public static Document getTrivialProject(final Document pipelineEntry) {
        final Document proj = (Document)pipelineEntry.get((Object)MongoPipelineOperators.PROJECT.getOperator());
    ...
    private static Document getMatch(final Document pipelineEntry) {
        return (Document)pipelineEntry.get((Object)MongoPipelineOperators.MATCH.getOperator());
    }
    ...
    public boolean needsCoercion() {
        for (final Document pipe : this.getPipelines()) {
            if (pipe.get((Object)MongoPipelineOperators.UNWIND.getOperator()) != null) {
                return true;
            }
            final Document projectDoc = (Document)pipe.get((Object)MongoPipelineOperators.PROJECT.getOperator());
    ...
    private static Document buildFilters(final Document pushdownFilters, final Map<String, List<Document>> mergedFilters) {
        ...
        for (final Map.Entry<String, List<Document>> entry : mergedFilters.entrySet()) {
            ...
            if (list.size() == 1) {
                ...
                Document bound = (Document)list.get(0).get((Object)fieldName);
                rangeQuery.putAll((Map)bound);
                bound = (Document)list.get(1).get((Object)fieldName);
                rangeQuery.putAll((Map)bound);
                listToAnd.add(new Document(fieldName, (Object)rangeQuery));
            }
        }
        ...
        if (listToAnd.size() > 0) {
            ...
            andQueryFilter.put(MongoFunctions.AND.getMongoOperator(), (Object)listToAnd);
    ...
    static {
        logger = LoggerFactory.getLogger((Class)MongoPipeline.class);
    }
}
