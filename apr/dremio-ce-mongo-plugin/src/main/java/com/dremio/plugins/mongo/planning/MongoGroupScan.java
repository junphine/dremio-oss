...
public class MongoGroupScan extends AbstractGroupScan
{
    ...
    public SubScan getSpecificScan(final List<SplitWork> work) throws ExecutionSetupException {
        ...
        if (this.isSingleFragment) {
            splitWork = (List<MongoSubScanSpec>)ImmutableList.of(new MongoSubScanSpec(this.spec.getDbName(), this.spec.getCollectionName(), ImmutableList.of(), null, null, this.spec.getPipeline().copy()));
        }
        else {
