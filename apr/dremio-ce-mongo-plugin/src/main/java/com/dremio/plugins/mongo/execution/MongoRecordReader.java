...
public class MongoRecordReader extends AbstractRecordReader
{
    ...
    protected Collection<SchemaPath> transformColumns(final Collection<SchemaPath> projectedColumns) {
        final Set<SchemaPath> transformed = Sets.newLinkedHashSet();
        if (!this.isStarQuery()) {
    ...
    private void init(final MongoSubScanSpec subScanSpec, final boolean isSingle) {
        ...
        this.collection = (MongoCollection<RawBsonDocument>)db.getCollection(subScanSpec.getCollectionName(), RawBsonDocument.class);
    }
        ...
        try {
            ...
            if (!this.sanitizedColumns.isEmpty()) {
                if (this.isBsonRecordReader) {
                    ...
                    this.jsonReader = new JsonReader(this.managedbuf, Lists.newArrayList((Iterable)this.sanitizedColumns), sizeLimit, maxLeafLimit, this.enableAllTextMode, false, this.readNumbersAsDouble);
    ...
    public int next() {
        if (this.cursor == null) {
            ...
            MongoRecordReader.logger.debug("Took {} ms to get cursor", watch.elapsed(TimeUnit.MILLISECONDS));
        }
        ...
        try {
            ...
            MongoRecordReader.logger.debug("Took {} ms to get {} records", watch2.elapsed(TimeUnit.MILLISECONDS), docCount);
    ...
    static {
        logger = LoggerFactory.getLogger(MongoRecordReader.class);
