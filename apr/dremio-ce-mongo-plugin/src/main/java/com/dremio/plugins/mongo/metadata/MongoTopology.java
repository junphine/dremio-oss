...
public class MongoTopology implements Iterable<MongoTopology.ReplicaSet>
{
    ...
    private final List<ReplicaSet> replicaSets;
    ...
    public static MongoTopology getClusterTopology(final MongoConnectionManager manager, final MongoConnection client, final boolean canDbStats, final String dbStatsDatabase, final String authDatabase, final boolean readOnlySecondary) {
        ...
        final Document isMasterMsg = MongoDocumentHelper.runMongoCommand(db, (Bson)new Document("isMaster", 1));
        ...
        final boolean isMongos = "isdbgrid".equalsIgnoreCase(isMasterMsg.getString("msg"));
        if (!isMongos) {
            MongoTopology.logger.debug("Detected that we are working with mongod according to isMaster response {}", isMasterMsg);
            final ReplicaSet replica = getReplicaSet("singlenode", client.getAddress(), isMasterMsg, readOnlySecondary);
            final MongoVersion minVersion = getMinVersion(manager, authDatabase, Lists.newArrayList(new ReplicaSet[] { replica }));
            return new MongoTopology(Collections.singletonList(replica), isMongos, false, false, minVersion);
        }
        MongoTopology.logger.debug("Detected that we are working with mongos according to isMaster response {}", isMasterMsg);
        if (!canDbStats) {
        ...
        for (final Shard shard : shards) {
            ...
            final Document randomIsMaster = MongoDocumentHelper.runMongoCommand(randomDB, (Bson)new Document("isMaster", 1));
    ...
    private static boolean canConnectTo(final MongoDatabase db, final ServerAddress hostAddr) {
        ...
        try {
            ...
            MongoTopology.logger.warn("User does not have access to MongoD instance '{}' in a replica set, this will slow down performance. Entering single thread mode", hostAddr);
        }
        catch (MongoClientException | MongoServerException ex2) {
            
            final MongoException e = ex2;
            MongoTopology.logger.warn("User does not have access to MongoD instance '{}' in a replica set, this will slow down performance. Entering single thread mode", hostAddr, e);
        }
        return false;
    }
    ...
    private static boolean canGetChunks(final MongoConnection client) {
        try {
            ...
            final com.mongodb.client.MongoCollection<Document> chunksCollection = configDB.getCollection("chunks");
    ...
    private static ReplicaSet getReplicaSet(final String name, final ServerAddress host, final Document isMasterMsg, final boolean readOnlySecondary) {
        final ArrayList<String> hosts = (ArrayList<String>)isMasterMsg.get("hosts");
        if (hosts == null) {
            return new ReplicaSet(name, Collections.singletonList(new MongoServer(host, ServerRole.PRIMARY)));
        }
        final String primary = isMasterMsg.getString("primary");
    ...
    private static List<Shard> getShards(final MongoDatabase db) {
        final Document dbStats = MongoDocumentHelper.runMongoCommand(db, (Bson)new Document("dbStats", 1));
        ...
        for (final String hostEntry : raw.keySet()) {
            ...
            if (shard == null) {
                MongoTopology.logger.warn("Failure trying to get shard names from dbStats. String {} didn't match expected pattern of [name]/host:port,host:port...", shard);
                return Collections.emptyList();
            }
            shards.add(shard);
        }
    ...
    static {
        logger = LoggerFactory.getLogger(MongoTopology.class);
    }
