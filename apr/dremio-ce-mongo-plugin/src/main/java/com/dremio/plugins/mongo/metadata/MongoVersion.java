...
public class MongoVersion extends Version
{
    ...
    static MongoVersion getVersionForConnection(final MongoDatabase database) {
        ...
        try {
            ...
            version = Version.parse(versionResult.getString("version"));
        }
        catch (Exception e) {
            MongoVersion.logger.warn("Could not get the mongo version from the server. Defaulting to version: {}", MongoVersion.MAX_MONGO_VERSION, e);
        ...
        try {
            compatibilityResult = database.runCommand((Bson)new BsonDocument(Arrays.asList(new BsonElement("getParameter", (BsonValue)new BsonBoolean(true)), new BsonElement("featureCompatibilityVersion", (BsonValue)new BsonBoolean(true)))));
        }
        catch (Exception e2) {
        ...
        if (version.getMajor() == 3 && version.getMinor() == 4) {
            final Version compatibility = Version.parse(compatibilityResult.getString("featureCompatibilityVersion"));
        ...
        if ((version.getMajor() == 3 && version.getMinor() > 4) || version.getMajor() >= 4) {
            final Document featureCompatibilityVersion = (Document)compatibilityResult.get("featureCompatibilityVersion");
            final Version compatibility2 = Version.parse(featureCompatibilityVersion.getString("version"));
    ...
    static {
        logger = LoggerFactory.getLogger(MongoVersion.class);
